...
<style>
    ...
    /* VERA Top Decision Area */
    .vera-decision-container {
        display: flex;
        gap: 20px;
        margin-bottom: 20px;
        padding: 20px;
    ...
    .verdict-red { background: rgba(255, 75, 75, 0.2); color: #ff4b4b; border: 1px solid rgba(255, 75, 75, 0.4); }
    .verdict-yellow { background: rgba(255, 200, 0, 0.2); color: #ffc107; border: 1px solid rgba(255, 200, 0, 0.4); }
    .verdict-green { background: rgba(40, 167, 69, 0.2); color: #4cd964; border: 1px solid rgba(40, 167, 69, 0.4); }
    ...
    .action-allowed { color: #4cd964; opacity: 1.0; }
    .action-denied { color: #ff4b4b; opacity: 0.6; text-decoration: line-through; }
...
def render_vera_top_decision_area(result):
    """
    Renders the VERA Top Decision Area based on the API response.
    """
    ...
    if r_state == "RED": verdict_cls = "verdict-red"
    elif r_state == "GREEN": verdict_cls = "verdict-green"
    
    # Allowed Actions (Reordered: Roll -> CSP -> Buy)
    # Constraints HTML
    constraints = d.get("constraints", [])
    cons_html = ""
    if constraints:
        cons_items = "".join([f'<div style="background:rgba(255,100,100,0.15); color:#ffa5a5; padding:2px 6px; border-radius:4px; margin-bottom:4px; font-size:0.75rem;">{c}</div>' for c in constraints])
        cons_html = f'<div style="margin-top:12px; border-top:1px solid rgba(255,255,255,0.1); padding-top:8px;">{cons_items}</div>'

    # Next Conditions HTML (Progress)
    next_conditions = d.get("next_conditions_details", [])
    nc_html = ""
    if next_conditions:
        nc_rows = ""
        for c in next_conditions:
            icon = c.get("status", "❓")
            # Color logic: Green check, Red cross
            val_color = "#4cd964" if icon == "✅" else "#ff4b4b"
            nc_rows += f'<div style="display:flex; justify-content:space-between; align-items:center; font-size:0.8rem; margin-bottom:4px; color:#bbb;"><span>{icon} {c.get("label")}</span><span style="color:{val_color}; font-family:monospace;">{c.get("value")} <span style="color:#666;">/ {c.get("target")}</span></span></div>'
        nc_html = f'<div style="margin-top:10px; background:rgba(0,0,0,0.2); padding:8px; border-radius:6px;">{nc_rows}</div>'
    
    # Action Reasons Maps
    reasons = d.get("action_reasons", {})

    # Action Gate HTML (With Reasons)
    actions = d.get("allowed_actions", {})
    action_html = ""
    
    def _render_action(key, label):
        is_allowed = actions.get(key, False)
        # Reason Tag
        reason = reasons.get(key, "")
        # Styles
        icon = "✅" if is_allowed else "❌"
        cls = "action-allowed" if is_allowed else "action-denied"
        # Tag Style
        tag_bg = "rgba(76, 217, 100, 0.15)" if is_allowed else "rgba(255, 75, 75, 0.15)"
        tag_color = "#4cd964" if is_allowed else "#ff4b4b"
        tag_html = f'<span style="background:{tag_bg}; color:{tag_color}; padding:2px 6px; border-radius:4px; font-size:0.7rem; margin-left:auto;">{reason}</span>' if reason else ""
        
        return f'<div class="vera-action-item {cls}" style="display:flex; align-items:center; width:100%;">{icon} {label} {tag_html}</div>'

    action_html += _render_action("roll_put", "期权滚单 (Roll)")
    action_html += _render_action("sell_put_csp", "卖出 Put (CSP)")
    action_html += _render_action("buy_underlying", "买入正股/建仓")

    # Build Tooltip Content
    u_def = get_u_state_def(u_state)
    o_def = get_o_state_def(o_state)
    as_of = result.get("asof", "-")
    reasoning_help = f"""<b>认知预警 (Reasoning):</b> 基于 VERA 引擎对市场微观结构与期权异动的实时监控。<br><br><b>当前状态定义:</b><br>• <b>{u_label} ({u_state}):</b> {u_def}<br>• <b>{o_label} ({o_state}):</b> {o_def}<br><br><b>策略暗示:</b> {d.get('reason', '-')}"""
    
    # Help Overlay HTML
    help_html = f"""<div class="metric-help-overlay" style="bottom: 80px; right: 280px; width: 400px;">{reasoning_help}</div>"""

    # Layout Change: 3 Columns (Verdict+Regime+Constraints | Reasoning+Conditions | Action Gate)
    html_content = f"""
<div class="vera-decision-container">
<div class="vera-decision-col" style="flex: 0 0 180px; border-right: 1px solid rgba(255,255,255,0.1); padding-right: 20px;">
<div class="vera-decision-label">VERA 最终裁定</div>
<div class="vera-verdict-badge {verdict_cls}">{d.get("R_state_label", r_state)}</div>
<div style="font-size: 0.75rem; color: #6b7280; margin-top: 8px; text-align: center;">{u_label} | {o_label}</div>
{cons_html}
</div>
<div class="vera-decision-col vera-tooltip-trigger" style="border-right: 1px solid rgba(255,255,255,0.1); padding-right: 20px; padding-left: 20px; flex: 2; cursor: help; position: relative;">
<div class="vera-decision-label" style="display:flex; justify-content:space-between;">
    <span>认知预警 (Reasoning) <span style="font-size: 0.8em; opacity: 0.7;">ⓘ</span></span>
    <span style="font-size:0.75rem; color:#666;">Data: {as_of}</span>
</div>
<div style="font-size: 1.0rem; color: #e5e7eb; font-weight: 500;">{d.get('reason', '-')}</div>
...
<div class="vera-decision-col" style="padding-left: 20px;">
<div class="vera-decision-label">动作许可 (Action Gate)</div>
<div class="vera-action-list">
{action_html}
</div>
</div>
</div>
...
def render_behavior_flags_inline(data: DashboardData, vera_result: dict = None):
    """Compact behavior flags / VERA evidence display for top decision area"""
    # 1. Prefer VERA Evidence (Fact-based)
    if vera_result and "decision" in vera_result:
        evidence = vera_result["decision"].get("evidence", [])
        if evidence:
            for item in evidence:
                 # Simple fact style: Cyan/Teal for Facts
                 st.markdown(f"""
                    <div style="padding:4px 8px; margin-bottom:4px; background:rgba(0, 172, 193, 0.1); border-left:3px solid #00acc1; border-radius:2px;">
                        <div style="font-size:0.75rem; color:#e0f7fa; font-weight:500;">{item}</div>
                    </div>
                """, unsafe_allow_html=True)
            return

    # 2. Fallback to Legacy Behavior Flags
    flags = data.behavior_flags or []
    
    if not flags:
        st.markdown("<div style='padding:8px; color:#aaa; font-size:0.85rem;'>无执行告警</div>", unsafe_allow_html=True)
        return
...
def render_quality_overlay(data: DashboardData):
    ...
    template_name = q.get("quality_template_name", "General")
    template_label = "银行模板" if template_name == "Banks" else "通用模板"
    
    help_html = f'<div class="vera-help-icon" style="position:absolute; top:12px; right:12px; cursor:default; color:{quality_color}; opacity:0.8;">ⓘ</div>' \
                f'<div style="position:absolute; top:12px; right:40px; font-size:0.7rem; background-color:{quality_color}33; color:{quality_color}; padding:1px 8px; border-radius:12px; border:1px solid {quality_color}55;">{template_label}</div>' \
                f'<div class="metric-help-overlay" style="width:280px; text-align:left; line-height:1.4;">{help_text}</div>'

    # Combine into a single string with NO leading whitespace or triple-quote newlines
    buffer_html = f'<div style="position:relative; padding:16px; background-color:rgba(255,255,255,0.05); border-radius:8px; border-left:4px solid {quality_color};">' \
                  f'{help_html}' \
                  f'<div style="font-size:1.4rem; color:{quality_color}; font-weight:600; margin-bottom:8px; display:flex; align-items:center;">' \
                  f'{quality_icon} Quality Buffer: {translate(quality_level, "zh_only")}' \
                  f'</div>' \
                  f'<div style="font-size:0.9rem; color:#ccc; line-height:1.6;">{quality_summary}</div>' \
                  f'</div>'

    st.write(buffer_html, unsafe_allow_html=True)
...
def render_risk_overlay(data: DashboardData, vera_result: Optional[dict] = None):
    ...
    with c1:
        ...
        if vera_result and "decision" in vera_result:
             evidence = vera_result["decision"].get("evidence", [])
             if evidence:
                 ev_html = "".join([f'<span style="background:rgba(255,255,255,0.1); padding:2px 6px; border-radius:4px; font-size:0.8em; margin-right:4px;">{e}</span>' for e in evidence])
                 st.markdown(f'<div style="margin-top:6px; margin-bottom: 6px;">{ev_html}</div>', unsafe_allow_html=True)
...
def render_page(data: DashboardData, profile: Optional[RiskProfile] = None, chart_start_date=None, chart_end_date=None):
    ...
    section_title("🎯 VERA 核心决策 (Decision Engine)")
    
    vera_result = None
    try:
        from vera.interface import get_vera_verdict
        with st.spinner("Running VERA Engines..."):
             vera_result = get_vera_verdict(data.symbol)
    ...
    with c3:
        st.markdown("**执行告警 (Alerts)**")
        render_behavior_flags_inline(data, vera_result)
    ...
    render_risk_overlay(data, vera_result)
    divider()
...
def render_data_import_page():
    ...
    friendly_names = {
        ...
        "provision_expense": "信用减值损失",
        "net_profit": "归母净利润",
        
        # Balance Sheet
        "total_loans": "发放贷款及垫款总额",
        "loan_loss_allowance": "贷款减值准备余额",
        "npl_balance": "不良贷款余额",
        "npl_ratio": "不良贷款率",
        "common_equity_begin": "期初普通股股东权益",
        "common_equity_end": "期末普通股股东权益",
        "total_common_equity": "普通股股东权益(平均)",
        "total_assets": "资产总计",
        ...
        # Dividends & Cash Flow
        "dividends_paid": "已支付股利 (现金流)",
        "dividend_per_share": "每股分红 (DPS)",
        
        # Share Structure
        "shares_outstanding": "期末普通股总股数",
    ...
    with c1:
        import_type = st.radio("数据类型 (Type)", ["📈 行情 (Market)", "📋 财务 (Financial)", "📄 PDF/图片 (Document)"], horizontal=False)
    ...
    if 'import_result' in st.session_state:
        ...
        if res.get("status") == "saved":
            ...
            # B) Single File Result
                ...
                if "行情" in import_type:
                    ...
                    # Logic for Financials/Documents: Check for missing indicators
                    extracted_keys = set(res.get("extracted_data", {}).keys())
                    # Filter out metadata/auxiliary/legacy fields from expectations
                    ignore_keys = {"report_date", "pe_ttm", "pb", "dividend", 
                                  "shares_diluted", "treasury_shares", "dividend_yield", 
                                  "roae", "roaa", "shares_outstanding"} # shares_outstanding is often treated specially
                    
                    # Use friendly_names keys that are NOT in ignore_keys
                    ...
                    with st.expander("🛠️ 调试信息 (若提取结果为空，请检查此处)"):
        ...
        pending_docs_info = []
        if "details" in res:
            pending_docs_info = [d for d in res["details"] if d.get("status") == "success" and "extracted_data" in d]
        ...
        if pending_docs_info:
            ...
            if st.button("💾 确认所有修正并保存至数据库", type="primary"):
                with st.spinner("正在写入数据库并存证..."):
                    ...
                    for doc in pending_docs_info:
                        # In multi-file mode, we just use the current doc data. 
                        # In single file mode, we use the modified final_metrics_to_save.
        ...
        if "details" in res:
            ...
            # 2. File selector for detailed editing
            files_with_data = [d for d in res["details"] if "extracted_data" in d and d["extracted_data"]]
            if files_with_data:
                st.markdown("---")
                st.markdown("#### 📝 数值校验与修正")
                
                # Create file selector
                file_names = [d["file_name"] for d in files_with_data]
                if len(file_names) > 1:
                ...
                d = files_with_data[selected_idx]
                ...
                # Optional: Show raw text and debug logs in expanders
                col_debug1, col_debug2 = st.columns(2)
                with col_debug1:
                    if "raw_text" in d and d["raw_text"]:
                        with st.expander(f"📄 查看原始文本", expanded=False):
                            st.code(d["raw_text"], language=None)
                
                with col_debug2:
                    if "debug" in d and d["debug"]:
                        with st.expander(f"🛠️ 查看解析日志", expanded=False):

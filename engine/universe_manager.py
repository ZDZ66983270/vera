        ...
        return [
            {
                ...
                "industry_name": r[10],
                "benchmark_etf": r[11],
                "benchmark_index": r[12],
                "last_data_date": r[13],
                "data_duration_years": round(r[14], 2) if r[14] is not None else 0.0,
                "last_report_date": r[15],
                "report_duration_years": round(r[16], 2) if r[16] is not None else 0.0
            } for r in rows
        ]
    finally:
        if own_conn:
...
def add_to_universe(
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO assets (asset_id, name, market, asset_type, industry, tags)
            VALUES (?, ?, ?, ?, ?, ?)
        ...
        """, (canonical_id, name, final_market, final_type, industry_name or 'Unknown', tags))

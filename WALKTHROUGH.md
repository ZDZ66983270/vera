...
*   **四大象限 (Quadrant)**：
    ...
    *   **Q3 (恐慌区)**: 低位 + 高路径风险。
    *   **Q4 (稳态区)**: 低位 + 低路径风险。
    *  - **智能行情导入 (OCR)**: 实现了基于行情截图的 OCR 数据自动解析与入库。
    - **前端集成**: 在侧边栏新增了图片上传组件，支持直接识别截图并弹出确认表单进行核对（目前根据需求已隐藏入口）。
    - **解析算法**: 采用 Tesseract (chi_sim+eng) 引擎，针对主流行情软件（如富途、雪球、Mooin）的版面进行了正则提取适配。
- **数据一致性与后缀归一化**:
    - **自动合并**: 实现了 A 股后缀（.SH/.SS）数据的自动合并加载。无论用户输入哪个后缀，系统都会拉取全量历史数据进行分析。
    - **UI 纯净化**: 优化了符号显示逻辑，主界面和下拉列表中统一移除 A 股/港股的市场后缀，同时支持纯数字代码的智能识别。
- **算法鲁棒性加固**: 解决了银行资产质量指标的类型转换错误 (`Unknown format code 'f'`)，并优化了风险解释文案的排版对齐。
...
### 3. Recent Refactoring Log (Dec 23, 2025)
*   **Path Risk Logic Refinement**:
    *   Aligned Path Risk interpretation with D-states (D0-D6) for authoritative structural analysis.
    *   Unified drawdown progress using the Risk State Machine, eliminating redundant calculations.
    *   Implemented UI semantic labels ("结构高点", "结构触底") for 0% and 100% progress.
    *   Added logic to hide drawdown progress display when Path Risk is HIGH and near peak to avoid intuitive conflict.
*   **Dashboard Architecture Upgrade**:
    *   Completely refactored `DashboardData` to use a structured, nested model (Path, Position, Value, Overlay) instead of flat fields.
    *   Centralized view logic in `dashboard.py` (e.g., `_build_position_card`), removing ad-hoc rendering logic from `app.py`.
    *   Updated `app.py` to consume the new structured data model, improving maintainability and separation of concerns.
*   **Market Context Integration**:
    *   Implemented partial integration of Market Amplifier and Index Risk State into the Dashboard structure.
